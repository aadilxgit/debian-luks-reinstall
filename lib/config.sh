...
# Snapshot the caller's environment BEFORE defaults are applied below:
# config-file values must win over built-in defaults, and only variables the
# caller really exported may shadow the config file.
declare -A CONFIG_ENV_SET=()
for _reinstall_k in "${CONFIG_KEYS[@]}"; do
    # bash sets HOSTNAME automatically for every shell; it is not a real
    # environment override, so the config file must be able to set it.
    [[ $_reinstall_k == HOSTNAME ]] && continue
    [[ -n ${!_reinstall_k+x} ]] && CONFIG_ENV_SET[$_reinstall_k]=1
done
unset _reinstall_k
...
apply_env_overrides() { local k; for k in "${CONFIG_KEYS[@]}"; do if [[ ${CONFIG_ENV_SET[$k]:-0} == 1 ]]; then printf -v "$k" '%s' "${!k}"; fi; done; }
set_defaults() { :; }
collect_ssh_keys() {
