...
disk_collect() { local src pk; src=$(findmnt -no SOURCE /); src=${src%%[*}; pk=$(lsblk -no PKNAME "$src" 2>/dev/null | head -n1); if [[ -n $pk ]]; then TARGET_DISK=/dev/$pk; else TARGET_DISK=$src; fi; }
...
guard_virtualization() { if command -v systemd-detect-virt >/dev/null 2>&1; then if systemd-detect-virt --container >/dev/null 2>&1; then die "cannot reinstall from inside a container ($(systemd-detect-virt)); aborting."; fi; else if [[ -e /proc/vz || $(tr '\0' '\n' </proc/1/environ) == *container=* ]]; then die "cannot reinstall from inside a container; aborting."; fi; fi; }
