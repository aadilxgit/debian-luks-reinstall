...
guard_virtualization() { if command -v systemd-detect-virt >/dev/null 2>&1; then systemd-detect-virt --container >/dev/null 2>&1 && die "kexec cannot run inside a container ($(systemd-detect-virt)); aborting."; else [[ -e /proc/vz || $(tr '\0' '\n' </proc/1/environ) == *container=* ]] && die "kexec cannot run inside a container; aborting."; fi; }
