...
redact_add(){ REDACT+=("$1"); }; die(){ log_error "$*"; return 1; }; require_root(){ [[ $(id -u) == 0 ]] || die "must run as root"; }; require_cmd(){ local c; for c; do command -v "$c" >/dev/null || die "missing command: $c"; done; }; confirm_yes(){ local prompt=$1 a; printf '%s (type YES): ' "$prompt" >&2; read -r a || return 1; [[ $a == YES ]]; }
